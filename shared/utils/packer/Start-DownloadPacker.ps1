...
$packer_bin = Get-ChildItem -Path $PSScriptRoot | Where-Object { $_.Extension -eq ".exe" } | Sort-Object | Select-Object -First 1 *
...
foreach ($link in $download_links) {
    if (($link -match "$env:PROCESSOR_ARCHITECTURE") -and ($link -match "$os") -and ($link -match "$packer_latest_release" )) {
        ...
        if ($null -eq $packer_bin) {
            $packer_bin = Get-ChildItem -Path $PSScriptRoot  | Where-Object { $_.Extension -eq ".exe" } | Sort-Object | Select-Object -Last 1 *
        }
